...
def geopotential_at_model_levels(dlnp,
                                 Tv,
                                 Z0,
                                 alpha,
                                 levs,
...
def geostrophic_wind_at_model_levels(lat, Z, Tv, dp, dlnp, alpha, upper):
    ...
    f = 2 * Omega * np.sin(
        lat * np.pi / 180.)[None, None, :, None]  # Coriolis parameter (1/s)
    mapfac = np.cos(lat * np.pi / 180.)[None, None, :,
                                        None]  # map scale factor
...
def momentum_tendencies_at_ml(lat, Z, Tv, dp, dlnp, alpha, upper, u, v):
    ...
    adv_u = -1 / (a * mapfac**2) * (u * np.gradient(u, dx, axis=3) +
                                    mapfac * v * np.gradient(u, dy, axis=2))
    adv_v = -1 / (a * mapfac**2) * (u * np.gradient(v, dx, axis=3) +
                                    mapfac * v * np.gradient(v, dy, axis=2))
    ...
    pgf_u = -1 / a * (np.gradient(Z, dx, axis=3) + Rd * Tv / dp *
                      (dlnp * np.gradient(upper, dx, axis=3) +
                       alpha * np.gradient(dp, dx, axis=3)))
    pgf_v = -mapfac / a * (np.gradient(Z, dy, axis=2) + Rd * Tv / dp *
                           (dlnp * np.gradient(upper, dy, axis=2) +
                            alpha * np.gradient(dp, dy, axis=2)))
...
if __name__ == "__main__":
    ...
    era5 = xr.open_dataset(
        'era5_201006_ml.nc')  #.sel(level=slice(None,124,-1))
    ...
    z = Z / 9.81
    Ug, Vg = geostrophic_wind_at_model_levels(lats, Z, Tv, dp, dlnp, alpha,
                                              upper)
    Ua = Ug - era5.u
    Va = Vg - era5.v
